...
def knapsack_multichoice_onepick(weights, values, max_weight, num_results=1, verbose=False):
    if len(weights) == 0:
        return 0
    ...
    solutions = get_onepick_solutions(last_array, last_path, num_results)

    return solutions


def get_onepick_solutions(scores, paths, num_results=1):
    scores_paths = list(zip(scores, paths))
    scores_paths_by_score = sorted(scores_paths, key=lambda tup: tup[0], reverse=True)

    # Get the top num_results solutions
    if num_results <= 0:
        top_solutions = scores_paths_by_score
        return [(score, path) for score, path in top_solutions if score != -1]
    else:
        top_solutions = scores_paths_by_score[:num_results]
        return [(score, path) for score, path in top_solutions] # if score != -1]
...
# print(knapsack_multichoice_onepick(weights, values, W, 5))  # [(15, [(0, 1), (1, 1), (2, 1)]), (14, [(0, 1), (1, 1), (2, 0)]), (11, [(0, 0), (1, 1), (2, 1)]), (-1, []), (-1, [])]
